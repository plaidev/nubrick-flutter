...
.PHONY: reinstall-ios
reinstall-ios:
	cd example && flutter clean && rm -rf ./ios/Pods ./ios/Podfile.lock 2> /dev/null && pod repo update && pod install && flutter build ios --simulator
	cd e2e && flutter clean && rm -rf ./ios/Pods ./ios/Podfile.lock 2> /dev/null && pod repo update && pod install && flutter build ios --simulator
